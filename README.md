...
投稿前にメッセージはサニタイズされます。Slackのリンク記法 (`<https://example.com>` または `<https://example.com|label>`、`http`/`https` スキームのみ) だけはそのまま通し、それ以外の `&`、`<`、`>` はすべてエスケープされます。これにより `<!channel>`、`<!here>`、`<@USER_ID>`、`<#CHANNEL_ID>` のようなSlackマークアップは、Slack側でメンションやチャンネル参照として解釈されず、そのままリテラルなテキストとして表示されます (リンクのラベル部分に `<` や `>` を含めることもできないため、偽装リンクでマークアップを紛れ込ませることもできません)。
